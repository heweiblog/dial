...
func (h Hand) AddIpSec(ipsec *com.SysIpSec, interval int32) (r com.RetCode, err error) {
	...
	if elem, ok := IpsecMap[ipsec.RecordId]; ok {
		...
		log.Cfglog.Println("add ipsec success", ipsec)
		return com.RetCode_OK, nil
	}
	log.Cfglog.Println("add ipsec failed", ipsec)
	return com.RetCode_FAIL, nil
}

func (h Hand) DelIpSec(ipsecid string) (r com.RetCode, err error) {
	delete(IpsecMap, ipsecid)
	log.Cfglog.Println("del ipsec success", ipsecid)
	return com.RetCode_OK, nil
}
