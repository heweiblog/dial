...
func NewCollectProcessor(handler Collect) *CollectProcessor {
	...
	self364 := &CollectProcessor{handler: handler, processorMap: make(map[string]thrift.TProcessorFunction)}
	self364.processorMap["registerModule"] = &collectProcessorRegisterModule{handler: handler}
	self364.processorMap["unRegisterModule"] = &collectProcessorUnRegisterModule{handler: handler}
	self364.processorMap["heartBeat"] = &collectProcessorHeartBeat{handler: handler}
	self364.processorMap["reportTaskProcess"] = &collectProcessorReportTaskProcess{handler: handler}
	self364.processorMap["reportResult"] = &collectProcessorReportResult_{handler: handler}
	return self364
}
...
func (p *CollectProcessor) Process(iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	x365 := thrift.NewTApplicationException(thrift.UNKNOWN_METHOD, "Unknown function "+name)
...
func (p *CollectReportResultArgs) readField4(iprot thrift.TProtocol) error {
	...
	p.ResultList = tSlice
	for i := 0; i < size; i++ {
		_elem366 := &DomainResult_{}
		if err := _elem366.Read(iprot); err != nil {
			return thrift.PrependError(fmt.Sprintf("%T error reading struct: ", _elem366), err)
		}
		p.ResultList = append(p.ResultList, _elem366)
	}
